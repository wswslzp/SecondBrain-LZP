---
title: "Wiki Log"
---

# Log

Chronological record of all wiki operations.

## [2026-04-11] init | Second Brain created

Vault initialized with LLM Wiki pattern. Directories: `raw/`, `wiki/`, `sources/`. Schema defined in `CLAUDE.md`.

## [2026-04-11] ingest | AI救活了一家马桶公司，也点燃了存储芯片超级周期

Source: 硅谷101 YouTube. Created: `sources/ai-storage-chip-super-cycle.md`. Updated: `wiki/ai-impact-on-industry.md`.

## [2026-04-11] ingest | Claude + Obsidian have to be illegal

Source: @defileo Twitter thread. Created: `sources/claude-obsidian-second-brain.md`, `wiki/llm-wiki-pattern.md`, `wiki/andrej-karpathy.md`.

## [2026-04-11] ingest | HOW TO 1000x YOUR LUCK

Source: @digiii Twitter thread. Created: `sources/how-to-1000x-your-luck.md`. Updated: `wiki/four-kinds-of-luck.md`, `wiki/game-theory.md`.

## [2026-04-11] ingest | Luck and the Entrepreneur: The Four Kinds of Luck

Source: Marc Andreessen blog (pmarchive). Created: `sources/luck-and-the-entrepreneur.md`, `wiki/marc-andreessen.md`. Updated: `wiki/four-kinds-of-luck.md`.

## [2026-04-11] ingest | AI正在摧毁科技互联网行业，让程序员大批量失业？

Source: 立党 YouTube. Created: `sources/ai-destroying-programmer-jobs.md`. Updated: `wiki/ai-impact-on-industry.md`.

## [2026-04-18] ingest | A Motorcycle for the Mind

Source: Naval Podcast (nav.al/ai). Created: `sources/a-motorcycle-for-the-mind.md`, `wiki/naval-ravikant.md`, `wiki/vibe-coding.md`, `wiki/ai-for-learning.md`. Updated: `wiki/ai-impact-on-industry.md`.

## [2026-04-18] ingest | CS 153 '26 Frontier Systems

Source: Anjney Midha Stanford 讲座 (YouTube). Created: `sources/cs153-frontier-systems.md`, `wiki/anjney-midha.md`, `wiki/frontier-systems.md`, `wiki/context-feedback-loops.md`, `wiki/compute-infrastructure.md`, `wiki/sovereign-ai.md`. Updated: `wiki/ai-impact-on-industry.md`.

## [2026-04-18] ingest | How to Get Rich (without getting lucky)

Source: Naval 系列播客 (nav.al/rich). Created: `sources/how-to-get-rich.md`, `wiki/wealth-creation.md`, `wiki/leverage.md`, `wiki/specific-knowledge.md`, `wiki/judgment.md`, `wiki/accountability.md`, `wiki/productize-yourself.md`. Updated: `wiki/four-kinds-of-luck.md` (加入 Naval 版本).

## [2026-04-18] ingest | Social Media Marketing is Dead, Neuromarketing is Next

Source: @karlocreates X thread. Created: `sources/neuromarketing-tribe-v2.md`, `wiki/neuromarketing.md`, `wiki/attention-economy.md`. Updated: `wiki/ai-impact-on-industry.md`.

## [2026-04-18] update | Index 重组

`index.md` 重新组织，新增 4 个 sources、2 个 entities（Naval, Anjney Midha）、13 个新 concepts。Concepts 分类从"按学科"转为"按主题簇"（财富框架、AI 基础设施、注意力经济等）。

## [2026-04-18] ingest | CS153 Amit Jain (Luma AI) on Unified Intelligence

Source: Stanford CS 153 第 3 周（YouTube）. Created: `sources/cs153-luma-amit-jain.md`, `wiki/amit-jain.md`, `wiki/unified-models.md`, `wiki/multimodal-ai.md`, `wiki/ai-factory.md`, `wiki/cascaded-vs-fused-architectures.md`.

## [2026-04-18] ingest | CS153 Andreas Blattmann (BFL) on Visual Intelligence

Source: Stanford CS 153 第 2 周（YouTube）. Created: `sources/cs153-bfl-andreas-blattmann.md`, `wiki/andreas-blattmann.md`, `wiki/visual-intelligence.md`, `wiki/distillation.md`, `wiki/open-weights-strategy.md`.

## [2026-04-18] ingest | CS153 Mati Staniszewski (ElevenLabs) on Audio Intelligence

Source: Stanford CS 153 第 2 周（YouTube）. Created: `sources/cs153-elevenlabs-mati-staniszewski.md`, `wiki/mati-staniszewski.md`, `wiki/audio-intelligence.md`.

## [2026-04-18] update | CS 153 系列交叉引用

更新 `wiki/frontier-systems.md` 和 `wiki/context-feedback-loops.md` 以链接三个新讲座的 context 和架构实例。Index 新增 6 个 concepts（unified-models、multimodal-ai、visual-intelligence、audio-intelligence、cascaded-vs-fused-architectures、ai-factory、distillation、open-weights-strategy）和 3 个 entities（Amit Jain、Andreas Blattmann、Mati Staniszewski）。

## [2026-04-19] ingest | Using Claude Code: Session Management & 1M Context

Source: @trq212 (Thariq) X thread. Created: `sources/claude-code-session-management.md`, `wiki/context-management.md`, `wiki/context-rot.md`, `wiki/claude-code.md`. Updated: `wiki/llm-wiki-pattern.md`（加 runtime 与 context-management 引用）、`wiki/context-feedback-loops.md`（加术语澄清，区分 RL context 与 prompt context）。Index 新增 3 个 concepts（claude-code、context-management、context-rot）与 1 个 source。

## [2026-04-20] ingest | 3 Highly Useful Hermes Skills

Source: 0xJeff X article. Created: `sources/3-highly-useful-hermes-skills.md`, `wiki/hermes-agent.md`, `wiki/0xjeff.md`, `wiki/browser-harness.md`, `wiki/hindsight.md`. Topics: AI agent 实战经验，token 管理，工具安全警告。

## [2026-04-25] ingest | 认知科学中的人类记忆分类体系

Source: 自研综述（综合 Simply Psychology, Psychology Today, The Decision Lab, PMC/NIH 等）。Created: `sources/human-memory-cognitive-science.md`, `wiki/human-memory-systems.md`, `wiki/working-memory.md`, `wiki/forgetting-mechanisms.md`, `wiki/memory-consolidation.md`, `wiki/ai-agent-memory.md`. Topics: 多重存储模型、Baddeley 工作记忆、遗忘曲线、记忆巩固、与 AI Agent 记忆架构对比。

## [2026-04-25] ingest | The Last Job for Mankind: Context Farming

Source: Brett Goldstein (@thatguybg) X article. Created: `sources/context-farming.md`, `wiki/brett-goldstein.md`, `wiki/context-farming.md`, `wiki/agentic-micro-company.md`, `wiki/company-brain.md`, `wiki/composable-context.md`. Updated: `wiki/ai-agent-memory.md` (交叉引用). Topics: Context Farming、Agentic Micro Company、Company Brain、可组合全球上下文。

## [2026-04-26] ingest | SCM: Sleep-Consolidated Memory with Algorithmic Forgetting for LLMs

Source: Saish Shinde (Clyrai IP Studio), arXiv:2604.20943. Created: `raw/SCM Sleep-Consolidated Memory - Saish Shinde.md`, `sources/scm-sleep-consolidated-memory.md`, `wiki/sleep-consolidated-memory.md`, `wiki/saish-shinde.md`, `wiki/hebbian-learning.md`, `wiki/complementary-learning-systems.md`. Updated: `wiki/memory-consolidation.md` (加 SCM 实现与 CLS 引用), `wiki/ai-agent-memory.md` (加 SCM 和 CLS 相关项目). Topics: 脑科学启发的 LLM 睡眠记忆架构、NREM/REM 巩固、主动遗忘、4D 重要性评分、互补学习系统理论。

## [2026-04-26] analysis | Hermes vs SCM 记忆系统全方位对比

Created: `wiki/hermes-vs-scm-analysis.md`. 从架构哲学、模块映射、生命周期、CLS 理论视角对 Hermes 和 SCM 做了八个维度的深度对比。核心发现：Hermes 有完整的觉醒记忆工具但缺少离线巩固（"睡眠"），SCM 有完整的睡眠机制但只是记忆子系统。提出了最小可行"睡眠"方案和优先级建议。

## [2026-04-26] ingest | 多模态记忆系统调研：AI 记忆系统超越纯文本的前沿

Source: 自研调研（综合 arXiv 论文、GitHub 项目、脑科学文献）。Created: `raw/multimodal-memory-systems-research.md`, `sources/multimodal-memory-systems.md`, `wiki/multimodal-memory.md`, `wiki/memos-memory-os.md`, `wiki/simplemem.md`, `wiki/screenpipe.md`, `wiki/imagebind.md`, `wiki/cross-modal-binding.md`, `wiki/cognitive-atrophy.md`. Updated: `index.md`. Topics: 16个多模态记忆项目全景图（MemOS/SimpleMem/M3-Agent/MemVerse/Screenpipe/ImageBind等）、四种技术路线（嵌入统一/转写归一/MemCube/持续捕获）、脑科学多感官整合（γ振荡绑定、反向有效性）、认知萎缩风险、对 Hermes 的改进路线建议。

## [2026-04-26] ingest | Awesome-AI-Memory: 学术界 AI 记忆研究全景索引

Source: IAAR-Shanghai/Awesome-AI-Memory (GitHub). Created: `raw/awesome-ai-memory-repo.md`, `sources/awesome-ai-memory.md`, `wiki/ai-memory-taxonomy.md`, `wiki/memory-system-architecture.md`, `wiki/memory-forgetting.md`, `wiki/memory-retrieval-pipeline.md`, `wiki/memory-compression.md`, `wiki/ai-memory-benchmarks.md`, `wiki/ai-memory-open-source-landscape.md`, `wiki/iaar-shanghai.md`. Updated: `wiki/ai-agent-memory.md` (交叉引用), `wiki/forgetting-mechanisms.md` (交叉引用), `wiki/hindsight.md` (交叉引用), `index.md`. Topics: 349篇论文 + 97个开源项目的完整索引、记忆系统四层架构（存储→处理→检索→控制）、五种原子操作、记忆四维分类体系、遗忘机制四类、43个开源系统四代演化时间线（Zep→Mem0→MemOS→SkillClaw）、评测基准 53 篇、2026 年前沿方法（MemCoT/ClawVM/FadeMem/ShardMemo/OBLIVION）。

## [2026-04-26] ingest | AI 的"睡眠记忆处理"：从认知科学到开源实现

Source: 自研调研（综合 20+ 项目/论文、脑科学文献）。Created: `raw/AI睡眠记忆处理-从认知科学到开源实现 - Hermes Research.md`, `sources/ai-sleep-memory-research-survey.md`, `wiki/letta-sleep-time-compute.md`, `wiki/catastrophic-forgetting.md`, `wiki/generative-agents.md`. Updated: `wiki/ai-agent-memory.md` (新增 Letta/Generative Agents/灾难性遗忘引用), `wiki/sleep-consolidated-memory.md` (新增调研交叉引用). Topics: 人类睡眠记忆处理机制（SWS巩固+REM联想+突触缩放）、两大技术流派（神经网络睡眠模拟 vs LLM Agent 离线记忆管理）、三大核心项目（Letta Sleep-Time Compute ⭐21.7K / SCM 五模块 / OpenClaw Auto-Dream ⭐562）、灾难性遗忘与 CLS 互补学习系统、Stanford Generative Agents 记忆架构先驱、Hermes 零侵入实现路径。

## [2026-05-17] ingest | How to Build an AI Quant System. Test 1,000 Strategies per Week.

Source: @zostaff X long-form article (2026-05-13, 328K Views). Created: `raw/How to Build an AI Quant System - zostaff.md`, `sources/ai-quant-system-zostaff.md`, `wiki/zostaff.md`, `wiki/ai-quant-system.md`, `wiki/deflated-sharpe-ratio.md`, `wiki/backtest-leakage.md`, `wiki/walk-forward-validation.md`, `wiki/three-gate-evaluator.md`, `wiki/multi-agent-research-loop.md`. Updated: `index.md` (+ Entities: zostaff; + new Concepts category "量化金融与 AI 投研" with 6 pages; + Sources entry). New concepts span quantitative trading, multi-agent LLM research loops, statistical validation (DSR), and a portable "three-gate" decision pattern applicable beyond finance.


## [2026-05-18] ingest | 光互联大迁徙 - 从光纤、光模块、1.6T 到 CPO 与 OIO 的全产业链终局推演

Source: 吴梓豪leslie 知识星球「半导体大佬的会议室」长文 (2026-04-17). Created: `raw/光互联大迁徙 - 吴梓豪leslie.md`, `sources/optical-interconnect-migration-leslie.md`, `wiki/wu-zihao-leslie.md`, `wiki/control-point-convergence.md`, `wiki/cpo-co-packaged-optics.md`, `wiki/silicon-photonics.md`, `wiki/optical-interconnect-roadmap.md`. Updated: `index.md` (+ Entity: wu-zihao-leslie; + new Concepts category "半导体与光通信" with 4 pages; + Sources entry). 

核心要点：作者从全产业链视角推演光互联未来十年演进，提出「控制点收敛」框架——价值链最终向系统架构者(NVIDIA/Broadcom)、Foundry 制造平台(TSMC)、先进封装收敛。三极论：6.4T CPO 时代的 NVIDIA + Broadcom + TSMC。主动去神化 A 股光模块龙头与 FiconTEC 等炒作叙事。⚠️ 作者 95% 重仓 TSMC，疑似台积电背景，对 Foundry 层判断存在 confirmation bias，时间表预测可能偏乐观。

## [2026-06-09] ingest | 拆解机器人「肉身」：量产与供应链（硅谷101）

Source: 硅谷101 YouTube（https://www.youtube.com/watch?v=DZIhQeFEdXI）. Created: `sources/silicon-valley-101-humanoid-robot-hardware.md`, `wiki/humanoid-robot-hardware.md`, `wiki/robot-actuators.md`, `wiki/embodied-ai-supply-chain.md`. Updated: `index.md`.

## [2026-06-01] ingest | LLM 推理是怎么跑的：跟着 SGLang Omni 团队的设计思路走一遍

Source: 鸭哥每日手记 / Superlinear Academy. Created: `raw/LLM 推理是怎么跑的：跟着 SGLang Omni 团队的设计思路走一遍 - 鸭哥每日手记.md`, `sources/sglang-omni-llm-inference.md`, `wiki/sglang-omni.md`, `wiki/llm-inference-systems.md`, `wiki/multi-stage-decode.md`. Updated: `wiki/audio-intelligence.md`, `wiki/cascaded-vs-fused-architectures.md`, `wiki/multimodal-ai.md`, `index.md`.

## [2026-07-19] ingest | How to be more agentic（Cate Hall）

Source: Cate Hall, usefulfictions.substack.com (2024-01-11；2026 成书《You Can Just Do Things》). Created: `sources/how-to-be-more-agentic.md`, `wiki/agency.md`, `wiki/cate-hall.md`. Updated: `wiki/four-kinds-of-luck.md`（「增加运气接触面」= agency 版），`index.md`（+ Entities: cate-hall；+ 新 Concepts 子类「能动性与通才」；+ Sources）. 核心：能动性非天生而是可习得的技能；radical agency = 寻找 real edges（别人因烦人/不适而回避的事）；6 个 hacks（主动求拒绝/匿名反馈/增加运气接触面/假设一切可习得/热爱低地位护城河/别太辛苦），"Burnout is the ultimate agency-killer"。

## [2026-07-19] ingest | A reading list for generalists（Dylan Bowman）

Source: Dylan Bowman, LessWrong (2026-06-29). Created: `sources/a-reading-list-for-generalists.md`, `wiki/generalist.md`, `wiki/dylan-bowman.md`. Updated: `index.md`. 核心：强通才作为稀缺角色（AI safety 社区默认无人 own 的项目需通才扛起）；18 篇文章分四类（倾向性/策略/项目领导力/人际组织）；最推荐 Anna Salamon《Humans are not automatically strategic》；被引最多的作者 Paul Graham(5)、Ben Kuhn(4)。与 [[agency]] 互链（Cate Hall 一文即收录于本清单 Dispositional 类）。

## [2026-07-19] ingest | The most profitable skill of the 21st century (not AI)（Dan Koe）

Source: Dan Koe (@thedankoe), X long-form letter (2026-07-04). Created: `sources/most-profitable-skill-dan-koe.md`, `wiki/human-nature-meta-skill.md`, `wiki/dan-koe.md`. Updated: `wiki/neuromarketing.md`、`wiki/attention-economy.md`（related + 「同一母题的另一路径」段落），`index.md`. 核心：唯一不过时的 meta-skill 是理解人性；The mind is a story engine；Three Tensions（Survival/Identity/Progress）× Five Levers（Name the Threat/Mirror Identity/Exclude People/Paint the Transformation/Give the First Step）；Eugene Schwartz 的 5 levels of awareness、Zeigarnik effect。与 [[neuromarketing]] 互为「进化心理 vs 神经扫描」两条通往同一结论的路径。

## [2026-07-19] ingest | 立党AI学习研究完整教程（第一期）

Source: 立党 lidang, YouTube (2026-06-23). Created: `sources/lidang-ai-agent-tutorial.md`, `wiki/lidang.md`, `wiki/build-your-own-agent.md`, `wiki/headless-agent-pattern.md`, `wiki/agent-comfort-zone.md`, `wiki/multi-agent-patterns.md`. Updated: `wiki/claude-code.md`（两条使用路线）、`wiki/multi-agent-research-loop.md`（有产出的 multi-agent 正例）、`wiki/ai-impact-on-industry.md`（lidang 实体 + tasks vs jobs）、`sources/ai-destroying-programmer-jobs.md`（同作者互链），`index.md`（+ Entity: lidang；+ 新 Concepts 子类「AI Agent 工程与实践」）. 五部分：选模型(85 vs 95 分性价比)/手写最小 SWE agent/非程序员 headless 用法（temp folder + JSON 状态）/agent 舒适区（封闭可验证：编程·Lean4·EDA·MATLAB·CAD）与能力边界（不能违反因果论/信息论）/multi-agent 适用（map-reduce·go-driven）与天坑（组织架构模拟）。

## [2026-07-19] ingest | Inside Anthropic, the $965 Billion AI Juggernaut（Bloomberg The Circuit）

Source: Bloomberg Originals / Emily Chang × Dario & Daniela Amodei, YouTube (2026-06-10；raw 文件此前已入库但未消化). Created: `sources/inside-anthropic-the-circuit.md`, `wiki/anthropic.md`, `wiki/dario-amodei.md`, `wiki/daniela-amodei.md`, `wiki/scaling-laws.md`, `wiki/claude-constitution.md`, `wiki/tasks-vs-jobs.md`. Updated: `wiki/claude-code.md`（Anthropic/Boris Cherny 段）、`wiki/ai-impact-on-industry.md`（SaaSpocalypse + tasks vs jobs）、`wiki/context-farming.md`（tasks-vs-jobs 互链），`index.md`（+ Entities: anthropic/dario-amodei/daniela-amodei；+ Concepts: scaling-laws/claude-constitution/tasks-vs-jobs）. 核心：2021 年 7 位前 OpenAI 成员创立、co-founder 全留任；scaling laws 与 smooth exponential；enterprise-over-consumer 押注与首次盈利；Claude Cowork 引发 $285B SaaSpocalypse；Constitution 与 professional warmth；Dario 的 tasks vs jobs 与「50% entry-level 白领岗」警告；自比 Leo Szilard 而非 Oppenheimer。

## [2026-07-19] ingest | Jensen Huang: NVIDIA — The $4 Trillion Company & the AI Revolution（Lex Fridman #494）

Source: Lex Fridman Podcast #494（guest Jensen Huang），YouTube (2026-03-24；raw 文件此前已入库但未消化). Created: `sources/jensen-huang-lex-fridman.md`, `wiki/jensen-huang.md`, `wiki/nvidia.md`, `wiki/extreme-co-design.md`, `wiki/cuda.md`, `wiki/token-economics.md`. Updated: `wiki/scaling-laws.md`（+ Jensen 的四条 scaling laws）、`wiki/tasks-vs-jobs.md`（+ Jensen↔Dario 对立视角与放射科医生反例）、`wiki/compute-infrastructure.md`、`wiki/ai-factory.md`（「AI factory」两种含义辨析）、`wiki/open-weights-strategy.md`（Nemotron）、`wiki/sovereign-ai.md`（China/国家视角）、`wiki/embodied-ai-supply-chain.md`（机器人用工具）、`wiki/vibe-coding.md`（coding=specification）、`wiki/optical-interconnect-roadmap.md`（NVLink/Amdahl）、`wiki/claude-code.md`（OpenClaw），`index.md`（+ Entities: jensen-huang/nvidia；+ Concepts: extreme-co-design/cuda/token-economics）. 核心：extreme co-design（跨 stack + speed of light + 提前押注硬件）；accelerated computing→AI factory 公司史（CUDA-on-GeForce 存亡赌注，市值一度跌到 $1.5B）；四条 scaling laws（pre/post/test-time/agentic，最终只受 compute 约束）；供应链/能源/中国与开源（Nemotron）/TSMC；moat=CUDA install base + ecosystem；token 工厂经济学与 $3T/$10T 论证；purpose≠tasks 的乐观就业观（放射科医生不减反增）；intelligence is a commodity 而该抬高 humanity；领导力（manifesting the future、systematic forgetting、die on the job）。是与 [[inside-anthropic-the-circuit]] 就 tasks vs jobs 形成正面对位的旗舰级素材。

## [2026-07-19] ingest | CS153 | Nikhyl Singhal（Skip）on Product Management in the AI Era

Source: Stanford CS153 Frontier Systems 第 9 讲（讲者 Nikhyl Singhal，主持 Mike Abbott），YouTube (2026-05-08). Created: `sources/cs153-skip-nikhyl-singhal.md`, `wiki/nikhyl-singhal.md`, `wiki/product-management-ai-era.md`, `wiki/career-as-chapters.md`. Updated: `wiki/frontier-systems.md`（+ 非技术一讲 + 来源 + Jensen/Nvidia 链接）、`wiki/tasks-vs-jobs.md`（+ Nikhyl 的中层经理分布结论）、`wiki/ai-impact-on-industry.md`（+ 产品与组织层）、`wiki/four-kinds-of-luck.md`、`wiki/vibe-coding.md`、`wiki/generalist.md`（related），`index.md`（+ Entity: nikhyl-singhal；+ 新 Concepts 子类「产品与职业」；+ Sources）. 核心：产品管理四阶段（PMF/流程协调/hypergrowth/晚期再造）；Google Hangouts 教训（解决真实客户问题、迭代速度、大公司需更快见效）；AI 把 product 从「information mover」重塑为「有 judgment 的 builder」，组织变平、30–70% 裁员与薪资飙升并存、中层经理最危险；职业=15–18 段「书的章节」，想你的第二份工作、be modern、networking、systems mindset、comfortable 就该走。是 CS153 系列第 5 篇、也是首篇非技术课，与 [[tasks-vs-jobs]] 强关联。

## [2026-07-19] ingest | CS153 | Jensen Huang on the Compute Behind Intelligence

Source: Stanford CS153 Frontier Systems 第 6 讲（讲者 Jensen Huang / NVIDIA），YouTube. Created: `sources/cs153-jensen-huang-compute.md`, `wiki/accelerated-computing.md`. Updated: `wiki/jensen-huang.md`（forecasting 方法 / suffering→resilience / mobile→robotics 最大错误）、`wiki/nvidia.md`（五大 foundation 域 BioNemo/Alpamayo/Groot/climate、NVIDIA 自身重度用 Anthropic/OpenAI token、出口管制立场）、`wiki/extreme-co-design.md`（RISC/Hennessy 血统 + 四代 roadmap）、`wiki/token-economics.md`（MFU 批判）、`wiki/compute-infrastructure.md`（大学 compute crunch + 1000× 能源）、`wiki/scaling-laws.md`（thinking=内部 token/tool use=外部 token）、`wiki/claude-code.md`、`wiki/frontier-systems.md`，`index.md`. 相对 [[jensen-huang-lex-fridman]]（Lex #494）新增/更强调面向学生的一面：计算 60 年来首次被重新发明、codesign 的 RISC 血统与「post general-purpose」、Hopper→Blackwell→Vera Rubin→Feynman 随 workload 共演化、MFU 批判与「unit of intelligence per watt」、能耗 1000×、开源动机与五大 foundation 域、芯片出口管制（「everybody should have AI, nobody should have nuclear bombs」）、大学 compute crunch。新建概念页 [[accelerated-computing]] 作为 [[extreme-co-design]] 的上游「why」。

## [2026-07-19] ingest | CS153 | Scott Nolan (General Matter) on Energy Bottlenecks

Source: Stanford CS153 Frontier Systems 第 7 讲（讲者 Scott Nolan / General Matter），YouTube. Created: `sources/cs153-scott-nolan-energy.md`, `wiki/scott-nolan.md`, `wiki/general-matter.md`, `wiki/ai-energy-bottleneck.md`. Updated: `wiki/compute-infrastructure.md`（能源=compute 之上的实体地基）、`wiki/frontier-systems.md`（土地/电力/外壳层）、`wiki/sovereign-ai.md`（related），`index.md`. 核心 five-whys：compute→power→电网（super-linear vs 停滞）→baseload 只有核能（5–10 年上量）→核燃料→enrichment（美国全球市占 <0.1%）→「enrichment is the bottleneck all the way through to AI on a five-year time frame」。含铀燃料五步供应链、stranded energy/Bitcoin 彩排（Crusoe→Stargate）、核能认知与德国反面教材、时间尺度（真正 hockey stick 在 2030s 初–2035）。与 [[nvidia]]/[[jensen-huang]] 的「power is the blocker」需求侧、[[compute-infrastructure]] 的「1 MW=1 MW 但你得先有电」互补。

## [2026-07-19] ingest | CS153 | Ben Horowitz (a16z) on Venture Capital Systems, Network Effects

Source: Stanford CS153 Frontier Systems 第 8 讲（讲者 Ben Horowitz / a16z，主持 Anjney Midha），YouTube. Created: `sources/cs153-ben-horowitz.md`, `wiki/ben-horowitz.md`, `wiki/network-effects.md`, `wiki/venture-capital-systems.md`. Updated: `wiki/marc-andreessen.md`（a16z 联创互链）、`wiki/tasks-vs-jobs.md`（Ben 反驳 Dario 的第三声音）、`wiki/frontier-systems.md`（补 4 Cs 的 Capital+Culture），`index.md`（+ 新 Concepts 子类「风险投资与商业策略」）. 核心：网络效应 N² 价值=最强 moat、bootstrap 冷启动最难（Bell 卖第一部电话）、a16z 把自身建成网络（HP briefing center hack、不发工资砸网络）；VC 作为系统（共享经济不共享控制、7 人对话组、第一支基金 1/4 押 Skype、founder-market fit、投人、对 AI-LBO 说不）；AI 反转「不能砸钱解决问题」、code/UI 不再是 moat、需求近乎无上限、Wall Street 买 narrative（SaaSpocalypse、Navan 供应链护城河）。补齐 [[frontier-systems]] 4 Cs 里此前较少覆盖的 Capital 与 Culture。

## [2026-07-19] ingest | CS153 | Satya Nadella (Microsoft) on Building the Frontier Ecosystem

Source: Stanford CS153 Frontier Systems 第 1 讲（讲者 Satya Nadella / Microsoft），YouTube. Created: `sources/cs153-satya-nadella-frontier-ecosystem.md`, `wiki/satya-nadella.md`, `wiki/microsoft.md`, `wiki/frontier-ecosystem.md`. Updated（并行 backlink）：`wiki/nvidia.md`、`wiki/extreme-co-design.md`（Maia 200 codesign）、`wiki/open-weights-strategy.md`（MAI licensed weights）、`wiki/sovereign-ai.md`（frontier-ecosystem 互为镜像）、`wiki/scaling-laws.md`（$1B OpenAI 赌注）、`wiki/tasks-vs-jobs.md`（displacement + 新 agency/wages）、`wiki/compute-infrastructure.md`（三大 workload）、`wiki/frontier-systems.md`，`index.md`. 核心：2019 押注 OpenAI $10 亿源于「prepared mind」+ Gates-Grove 生态 DNA，真正的赌注是 compute concentration；frontier ecosystem=让每家公司带自己复利的 IP（含 token capital）在前沿运营，用 hill-climbing machine（licensed MAI + 私有 RLE/gym + private evals）避免把价值泄露给 foundation model；Copilot 三形态 chat→Cowork→Scout(autopilot，Entra ID digital twin)，MXC 容器隔离 agent；unmetered intelligence + edge silicon（RTX SoC / petaflop dev box / GB300 桌面）；Project Solara 新 agent 形态；Maia 200 与自家+OpenAI 模型 codesign 跑 GPT-5.5，Cobalt ARM 用 Copilot traces 调优；Majorana 2 拓扑量子；electricity→light 与 growth mindset（Dweck/NVC/bounded rationality）；cognitive coverage 学习法。新建概念页 [[frontier-ecosystem]] 与 [[sovereign-ai]] 互为镜像。

## [2026-07-19] ingest | CS153 | Sam Altman (OpenAI) on Scale, AGI, and the Future of Everything

Source: Stanford CS153 Frontier Systems 第 2 讲（讲者 Sam Altman / OpenAI），YouTube. Created: `sources/cs153-sam-altman-scale.md`, `wiki/sam-altman.md`, `wiki/openai.md`, `wiki/intelligence-as-utility.md`. Updated（并行 backlink）：`wiki/scaling-laws.md`（scale=quality/emergent）、`wiki/tasks-vs-jobs.md`（「less of a jobs doomer」+ compute 分配）、`wiki/network-effects.md`（YC batch 网络效应）、`wiki/token-economics.md`（tokens-as-utility）、`wiki/compute-infrastructure.md`、`wiki/ai-energy-bottleneck.md`（shortage forever）、`wiki/open-weights-strategy.md`（democratization fork），`index.md`. 核心：scale=quality，最有意思的东西都来自 emergent-at-scale（AI/研究/经济/YC batch），场被「太确定 scaling 不行的一代科学家」拖累；OpenAI=研究实验室倒装成公司；ChatGPT 系统故事（GPT-3 API 无 traction→病毒→copywriting→用户拿 API key 聊天→3.5 instruction-following→「不太好却疯长=铁定爆款」→紧急建公司）；Codex/5.5 拐点，pipeline 会被 AI 重写（Sept 500k A100 当 research intern，2028-03 端到端研究员）；intelligence 是新 utility（卖 light 而非 electricity），tokens-as-utility vs Jensen 的 compute-as-utility；democratization fork（~80% 走民主路径 vs 集中）；ownership stake/citizens' wealth fund 优于 UBI，杠杆从 labor 转向 capital；compute「shortage forever」；反 LeCun「LLM 死路」；教育 3.5 年无系统性改变是他的 prediction error。新建概念页 [[intelligence-as-utility]]。

## [2026-07-19] ingest | CS153 | Joe Sullivan on The Road Ahead: Resilience Required

Source: Stanford CS153 Frontier Systems 第 3 讲（讲者 Joe Sullivan，前 Uber/Facebook 安全负责人 & Cloudflare CISO），YouTube. Created: `sources/cs153-joe-sullivan-resilience.md`, `wiki/joe-sullivan.md`, `wiki/cybersecurity-resilience.md`，`index.md`（+ 新 Concepts 子类「安全与韧性」）. 核心：政府↔科技交界的职业弧（DOJ 检察官→eBay/PayPal→Facebook/NSA→Uber→Cloudflare）；Uber 泄露案主线（2016 两名少年黑客经 bug bounty 报告 5700 万人数据，付 $10 万、法务/CEO 签字、派前 CIA 审讯者核实删除；2020 本人被控 obstruction/misprision；trial 输在 18 USC 1030「能否事后授权」novel question；2023-05 判 3 年 probation，法官「不是掩盖」+ 训斥检方「为何不告 CEO」；收到 200+ 封求情信=「自己的爱尔兰守灵」）；responsible disclosure（PayPal 2007 首创）与 bug bounty（Facebook 第三个）史；透明>掩盖（Cloudflare「谁写 blog post」）；resilience=会被打脸就得有预案，危机成败首看沟通；AI 时代安全（vibe-coding 代码量暴涨、非工程师上线漏洞、agents-as-toddlers 运行时异常检测、Anthropic「Mythos」cyber 模型 6 个月扩散窗口、ransomware 从国家攻击到产业、operational resilience 如 Jaguar Land Rover）。新建概念页 [[cybersecurity-resilience]]。

## [2026-07-19] ingest | CS153 | Amin Vahdat (Google) on The Discipline of Delivering Value per Gigawatt

Source: Stanford CS153 Frontier Systems 第 4 讲（讲者 Amin Vahdat / Google 基础设施 VP），YouTube. Created: `sources/cs153-amin-vahdat-gigawatt.md`, `wiki/amin-vahdat.md`, `wiki/google.md`, `wiki/value-per-gigawatt.md`. Updated（并行 backlink）：`wiki/jensen-huang.md`（「Jensen 的反面」）、`wiki/nvidia.md`（TPU-vs-GPU 非零和）、`wiki/compute-infrastructure.md`（供应/lead-time）、`wiki/ai-energy-bottleneck.md`（电网供给侧）、`wiki/token-economics.md`（value per dollar）、`wiki/extreme-co-design.md`（system balance/8i-8t）、`wiki/accelerated-computing.md`（专用化 demand-side），`index.md`. 核心：1 GW≈$40-50B，真正的指标是 value per dollar→happy DAU 而非 GW；MFU 低源于 MoE 下 system balance 失衡（Amdahl：FLOPs:HBM:网络 必须配比，100% MFU 不可能）；可靠性/goodput（五个 9 需 2N，半数电力闲置；同步训练一节点挂全挂）；access-over-reliability 新现象（前沿实验室宁要 2× 容量+3.65 天/年停机）；光路交换 OCS + 3D torus（136 MEMS 镜、秒级换下坏 rack、按 5 小时作业指向存储）；TPU 专用化 8i/8t（memory:compute:网络 比不同），TPU-vs-GPU 非零和、6 年折旧；供应链（内存产能不足、净新增 GW lead-time 2-3 年、utility 20 年 take-or-pay、<100MW stranded、serving 更 fungible）；反零和 coda（「没有 winner/loser，只有把事做成的人」）。被主持人称为「Jensen 的反面」——严谨系统纪律的对位。新建概念页 [[value-per-gigawatt]]。

## [2026-07-19] ingest | CS153 | Garry Tan & Diana Hu (YC) on The AI Native Company

Source: Stanford CS153 Frontier Systems 第 5 讲（讲者 Garry Tan + Diana Hu / Y Combinator），YouTube. Created: `sources/cs153-garry-tan-diana-hu-ai-native.md`, `wiki/garry-tan.md`, `wiki/diana-hu.md`, `wiki/y-combinator.md`, `wiki/ai-native-company.md`, `wiki/agentic-engineering-primitives.md`. Updated（并行 backlink）：`wiki/claude-code.md`（$200 Max plan software factory / ADHD CEO）、`wiki/multi-agent-patterns.md`（skill/resolver/skillify）、`wiki/venture-capital-systems.md`（The SAFE 标准化）、`wiki/network-effects.md`（YC batch）、`wiki/llm-wiki-pattern.md`（GBrain 扩展 Karpathy Knowledge Wiki）、`wiki/tasks-vs-jobs.md`（中层承压佐证）、`wiki/frontier-systems.md`（Capital 层），`index.md`. 核心：unit of production 变了（Garry 用 $200/mo Claude Code 5 天重建 Posterous，6 人做 $10M、$1-2M/人）；GStack「software factory」，plan-review skill→80-90% 覆盖破 slop，「boil the ocean」一人抵 500-1000 人；agentic 工程原语 skill（可调 code 的 markdown runbook）/resolver（惰性加载=组织架构）/skillify（一次性升级为带 unit test+LLM eval+trigger eval 的技能，10 步里只有 2 步是写 code）/GBrain（3 层记忆，在 Karpathy Knowledge Wiki 上加 vector search+RRF+graph db+epistemology）；原语↔组织同构（skill=员工、resolver=org chart、check-resolvable=审计）；AI-native 组织 open-loop→closed-loop（PID）、IC/DRI/AI-founder 三角色、taste&evals 不可委派（cross-modal eval：Opus/GPT-5.5/DeepSeek V4 互评）、forward-deployed wedge（Salient/HappyRobot/Reducto 一年 0→八位数）、last-batch 3x/3 月。GBrain 自指式扩展了本 vault 所用的 [[llm-wiki-pattern]]。新建概念页 [[ai-native-company]] 与 [[agentic-engineering-primitives]]。
